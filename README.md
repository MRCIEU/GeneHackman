...
| **`PIPELINE_DATA_DIR`** | Directory containing **`genomic_data/`** and **`LDSCORE/`** (from step 3). Download reference data from **`gs://genehackman/1.1.0/`** for this release. For Apptainer profiles, this directory also stores the image cache at **`PIPELINE_DATA_DIR/genomic_data/pipeline/genehackman_<version>.sif`**, where `<version>` comes from **`DOCKER_VERSION`** or defaults to **`Version:`** in [`DESCRIPTION`](DESCRIPTION). The Docker profile uses `mrcieu/genehackman:<version>` directly and does not build a SIF. |
...
| **`SNAKEMAKE_PROFILE`** | Snakemake profile directory. Default: **`snakemake/profiles/apptainer/`** (local Apptainer). Use **`snakemake/profiles/docker/`** for local Docker, or e.g. **`snakemake/profiles/slurm/`** on a cluster. |
...
QTL_DATA_DIR=/path/to/my_qtl_data/
SNAKEMAKE_PROFILE=snakemake/profiles/apptainer/  # or snakemake/profiles/docker/
```
...
- Default: **`SNAKEMAKE_PROFILE=snakemake/profiles/apptainer/`** (Apptainer on the current machine).
- Local Docker: **`SNAKEMAKE_PROFILE=snakemake/profiles/docker/`**. `run_pipeline.sh` starts Snakemake inside `mrcieu/genehackman:<version>` and skips SIF building.
...
**macOS, Linux, Slurm, or PBS:** see **[PLATFORM_SETUP.md](PLATFORM_SETUP.md)** for Docker, Apptainer/Lima, SIF cache layout, profiles, and cluster templates.
